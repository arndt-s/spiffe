...
The SPIFFE Broker API consists of a specific vector that allows extension. Any other means of extensions to this API is prohibited and instead, implementers should introduce new gRPC services, according to the [extensibility method](./SPIFFE_Broker_Endpoint.md#7-extensibility-and-services-rendered) outlined in the SPIFFE Broker Endpoint specification.
...
### 4.1 Referencing the Workload

The SPIFFE Broker API allows by default for workloads to be referenced by its process ID. This reference is local-only and thus, requires for the requester to be on the same node or have strong knowledge about the node and its state. Referencing the workload by a process ID while invoking the SPIFFE Broker API over network is not reccommended because the client is not being able to observe state changes (such as process ID re-use) in a timely manner.

Implementation-specific references may be supported by extending the Broker API with custom references. This should be a delicate decision as this prevents interoperability and portability. If a reference is proven to be common it should rather be added into the SPIFFE Broker API than added as a custom extension.

References should always point to existing workloads and implementations may deny requests if they cannot de-reference the workload and verify its existance. References by SPIFFE-ID or other identifiers that don't point to an existing instance of a workload are prohibited. See [Appendix B](#appendix-b-referencing-vs-mintingissuance).

// NOTE(Arndt): We can also require de-reference-ability?
...
Implementations MUST maintain a strict allow-only policy that prevents any caller that isn't authorized to leverage the SPIFFE Broker API.
...
## Appendix B. Referencing vs minting/issuance
